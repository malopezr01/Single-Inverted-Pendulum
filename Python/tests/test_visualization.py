...
class PlotTests(unittest.TestCase):
    def test_qt_window_range_and_no_redundant_redraw(self):
        ...
        window.handle_header(['Time','theta','x','u'])
        window._open_plot('theta')
        try:
            ...
            x,y=window.plot_series['theta'][1].getData()
            self.assertEqual(len(x),1001)
            self.assertEqual(y[-1],2000)
            self.assertAlmostEqual(window.plot_series['theta'][0].viewRange()[0][0],10)
            with patch.object(window.plot_series['theta'][1],'setData') as redraw:
                window._update_plots()
                redraw.assert_not_called()
            window.plot_window_spin.setValue(2)
            x,y=window.plot_series['theta'][1].getData()
            self.assertEqual(len(x),201)
            self.assertAlmostEqual(window.plot_series['theta'][0].viewRange()[0][0],18)
            self.assertAlmostEqual(window.plot_series['theta'][0].viewRange()[0][1],20)
            worker.session.buffer.clear()
            window._update_plots()
            self.assertIsNone(window.plot_series['theta'][1].getData()[0])
        finally:
            window.close()
            window.deleteLater()
            app.processEvents()
